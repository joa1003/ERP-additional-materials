...
#
# Formal h=12 production runner for:
#   - lcl_source
#   - direct_transfer
#   - fine_tuning
#   - cer_scratch_limited
#   - cer_scratch_full
#
# The runner reads the lead-specific formal config configs/forecasting_h12.yaml
...
# Exact governed h=12 counts from Sample construction production construction.
EXPECTED_H12_COUNTS: dict[str, dict[str, int]] = {
    "lcl_source": {
        "train": 46_696_036,
        "validation": 6_670_087,
        "test": 13_314_437,
    },
    "direct_transfer": {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Formal forecasting formal h=12 forecasting runner."
    )
    parser.add_argument(
        "--project-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Defaults to <project-root>/configs/forecasting_h12.yaml.",
    )
    parser.add_argument(
        "--strategy",
...
def load_canonical_settings(
    ...
    if horizon != 12:
        raise RuntimeError("This Formal forecasting runner currently authorises h=12 only.")
    ...
    lcl_index = resolve_project_path(
        project_root,
        data["sample_indexes"]["LCL_h12"],
    )
    cer_index = resolve_project_path(
        project_root,
        data["sample_indexes"]["CER_h12"],
    )
...
def train_complete_epoch(
    ...
    expected = EXPECTED_H12_COUNTS[spec.name]["train"]
...
def validate_complete_split(
    ...
    synchronise(device)
    elapsed = time.perf_counter() - start
    expected = EXPECTED_H12_COUNTS[spec.name]["validation"]
    if observation_count != expected:
        raise AssertionError(
...
def plot_learning_curve(
    ...
    for _, row in extension_rows.iterrows():
        axis.axvline(int(row["epoch"]), linestyle=":", linewidth=1)
    axis.set_title(f"Formal forecasting h=12 {strategy} — seed {seed}")
...
def evaluate_test_and_write_predictions(
    ...
    synchronise(device)
    elapsed = time.perf_counter() - start
    expected = EXPECTED_H12_COUNTS[spec.name]["test"]
...
def main() -> None:
    ...
    config_path = (
        ...
        else project_root / "configs/forecasting_h12.yaml"
    )
