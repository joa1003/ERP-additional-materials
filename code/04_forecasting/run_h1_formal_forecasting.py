...
#   - cer_scratch_full
#
# The runner reads the lead-specific formal config configs/forecasting_h1.yaml
...
EXPECTED_H1_COUNTS: dict[str, dict[str, int]] = {
    "lcl_source": {
        "train": 46_743_079,
        "validation": 6_670_856,
        "test": 13_316_661,
    },
    "direct_transfer": {
        "train": 0,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Lead specific forecasting formal h=1 forecasting runner."
    )
    parser.add_argument(
        "--project-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Defaults to <project-root>/configs/forecasting_h1.yaml.",
    )
    parser.add_argument(
        "--strategy",
...
def load_canonical_settings(
    ...
    if horizon != 1:
        raise RuntimeError("This Lead specific forecasting runner authorises h=1 only.")
    ...
    lcl_index = resolve_project_path(
        project_root,
        data["sample_indexes"]["LCL_h1"],
    )
    cer_index = resolve_project_path(
        project_root,
        data["sample_indexes"]["CER_h1"],
    )
...
def train_complete_epoch(
    ...
    expected = EXPECTED_H1_COUNTS[spec.name]["train"]
...
def validate_complete_split(
    ...
    synchronise(device)
    elapsed = time.perf_counter() - start
    expected = EXPECTED_H1_COUNTS[spec.name]["validation"]
    if observation_count != expected:
        raise AssertionError(
...
def plot_learning_curve(
    ...
    for _, row in extension_rows.iterrows():
        axis.axvline(int(row["epoch"]), linestyle=":", linewidth=1)
    axis.set_title(f"Formal forecasting h=1 {strategy} — seed {seed}")
...
def evaluate_test_and_write_predictions(
    ...
    synchronise(device)
    elapsed = time.perf_counter() - start
    expected = EXPECTED_H1_COUNTS[spec.name]["test"]
...
def main() -> None:
    ...
    config_path = (
        ...
        else project_root / "configs/forecasting_h1.yaml"
    )
